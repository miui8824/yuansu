...
Page({
  ...
  eventDraw() {
    ...
    util.RequestPost(reqObj, null, (res, message) => {
      ...
      if (res.resCode == '0000') {
        ...
        // console.log(this.data.copywriting, this.data.imgUrl, res.data.img)
        this.setData({
          painting: {
            width: 375,
            height: 555,
      ...
      if (res.resCode == '0000') {
        ...
        this.setData({
          ...
          if (this.data.gdActivityInfo && this.data.gdActivityInfo.onlineState == 1) {
            ...
            if (nowDate < this.data.gdActivityInfo.beginTime) {
              this.setData({
                playCountdown: 1,
              })
              setInterval(() => {
                this.countTime(this.data.gdActivityInfo.beginTime)
              }, 1000)
            } else if (nowDate > this.data.gdActivityInfo.beginTime && nowDate < this.data.gdActivityInfo.endTime) {
              this.setData({
                playCountdown: 2,
              })
              setInterval(() => {
                this.countTime(this.data.gdActivityInfo.endTime)
              }, 1000)
            }
          }
          //.. 初始化函数
          ...
          //是新人进来 开新人红包
          // if (_this.data.isnew == 1) {
          //   _this.isOpenRed();
          // }
        })
        //初始化富文本
        if(res.data.gdInfo.description!=null){
          WxParse.wxParse('article', 'html', res.data.gdInfo.description, _this, 0);
        }
      }
    })
  },
  ...
  goHome() {
    ...
    console.log('gohome状态',app.globalData.isFromDetail1);
    wx.switchTab({
      url: '/pages/home/home',
    },()=>{
  ...
  onShareAppMessage() {
    ...
    //..要加个 用户ID  this.data.userId  this.data.goodId  this.data.activityId
    return {
      title: this.data.gdInfo.title,
